...
def _guess_import_field(column_name: str) -> str:
    ...
    aliases = {
        ...
        "is_active": {"isactive", "active", "enabled", "published"},
...
def _looks_like_serialized_csv_row(cells: list[str]) -> bool:
    if len(cells) != 1:
        return False
...
def _split_serialized_csv_row(raw_value: str) -> list[str]:
...
def _normalize_csv_rows(rows: list[list[str]]) -> tuple[list[str], list[list[str]]]:
    if not rows:
        return [], []
    ...
    for raw_row in rows[1:]:
        ...
        # Skip accidental repeated header payload rows.
        if row and len(row) >= width:
            normalized_first = _normalize_column_name(row[0])
            header_signature = _normalize_column_name(",".join(header[: min(5, len(header))]))
            if normalized_first and normalized_first.startswith(header_signature):
                continue
...
def _coerce_table(uploaded_file) -> tuple[list[str], list[list[str]]]:
    ...
    if filename.endswith(".csv"):
        ...
        raw_rows = list(csv.reader(StringIO(decoded)))
        return _normalize_csv_rows(raw_rows)
    ...
    if filename.endswith(".xlsx") or filename.endswith(".xls"):
        ...
        raw_rows = [[str(cell or "").strip() for cell in row] for row in sheet.iter_rows(values_only=True)]
        return _normalize_csv_rows(raw_rows)
...
def run_bulk_import_preflight(uploaded_file) -> dict:
    ...
    if not columns and not rows:
        return {
            ...
            "field_mapping": {},
            "stats": {"rows": 0, "columns": 0, "image_coverage": 0, "confidence_score": 0},
        }
    ...
    image_cols = [
        c for c, field in guessed_mapping.items()
        if field in {"image_url", "image_urls"} or _normalize_column_name(c) in IMAGE_FIELD_ALIASES
    ]
    ...
    if title_col and title_col in column_index:
        title_idx = column_index[title_col]
    ...
    if price_col and price_col in column_index:
        price_idx = column_index[price_col]
        invalid_prices = 0
        for row in rows:
            raw_price = str(row[price_idx]).strip() if price_idx < len(row) else ""
    ...
    if stock_col and stock_col in column_index:
        stock_idx = column_index[stock_col]
        invalid_stock = 0
        for row in rows:
            raw_stock = str(row[stock_idx]).strip() if stock_idx < len(row) else ""
    ...
    image_coverage = 0
    if image_cols:
        image_indexes = [column_index[column] for column in image_cols if column in column_index]
        for row in rows:
            if any(idx < len(row) and str(row[idx]).strip() for idx in image_indexes):
                image_coverage += 1
    ...
    summary = (
        ...
        f"Import confidence is {confidence_score}% based on required fields, image coverage, and data cleanliness."
    )
    ...
    preview = [
        {
            columns[idx] if idx < len(columns) else f"Column {idx + 1}": str(value)[:120]
